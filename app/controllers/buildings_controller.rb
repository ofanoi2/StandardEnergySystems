class BuildingsController < ApplicationController
	...
	def show
	  ...
	  @meter_number = @building.meters.distinct.pluck(:meter_number).paginate(page: params[:page], per_page: 1)
	  ...
	  @meter_number.each do |variable|
	  	@id = @building.meters.where("sequence_number is not null and meter_number = :meter_number", meter_number: variable).pluck(:id) 
	  	@meter_location = @building.meters.distinct.where("sequence_number is not null and meter_number = :meter_number", meter_number: variable).pluck(:meter_location)
	  end
	end
